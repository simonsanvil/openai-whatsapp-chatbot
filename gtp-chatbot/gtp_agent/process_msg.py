...
def process_and_reply(
    prompt: str, chat_agent: GTPAgent, max_response_length: int = 150
):
    """
    ...
    if re.match("ask (" + "|".join(engines) + ") .+", prompt.lower()):
        ...
        if agent_to_prompt == "codex":
            logger.debug("Prompting codex agent")
    ...
    change_engine_match = re.match(
        ".*speak with (?P<word_after>[A-Za-z0-9-\-\_]+)", prompt.lower()
    )
